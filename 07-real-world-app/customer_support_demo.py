"""
Example 7: Real-world Customer Support Demo
...
def run_customer_support_examples():
    ...
    for i, scenario in enumerate(scenarios, 1):
        ...
        try:
            ...
            print(f"\n✅ Workflow completed!")
            print(f"Intent: {result['intent_type'].value if result['intent_type'] else 'Unknown'}")
            print(f"Priority: {result['priority_level'].value if result['priority_level'] else 'Unknown'}")
            print(f"Agent: {result['current_agent'].value if result['current_agent'] else 'Unknown'}")
            ...
            if result.get("performance_metrics"):
                ...
                print(f"Session Duration: {metrics.get('session_duration_seconds', 0):.2f}s")
                print(f"Follow-up Required: {metrics.get('follow_up_required', False)}")
